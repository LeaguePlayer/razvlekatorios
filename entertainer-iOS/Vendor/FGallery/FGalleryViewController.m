...
- (void)loadView
{
    ...
    [_toolbar setBackgroundImage:[UIImage imageNamed:@"bottom_bar.png"] forToolbarPosition:UIToolbarPositionAny barMetrics:UIBarMetricsDefault];
...
- (FGalleryPhoto*)createGalleryPhotoForIndex:(NSUInteger)index
{
    ...
    else if (sourceType == FGalleryPhotoSourceTypeImage){
        UIImage *image = [_photoSource photoGallery:self imageForPhotoSize:FGalleryPhotoSizeThumbnail atIndex:index];
        photo = [[FGalleryPhoto alloc] initWithImage:image fullsizeImage:image delegate:self];
    }
	else 
	{
